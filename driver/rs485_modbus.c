...
/*
 * rs485_modbus.c — Sensor Hub: Serdev RS485/Modbus + Cdev Driver (RPi4 v3)
 *
 * Full hardware driver for the Raspberry Pi 4 (BCM2711).
 *
 * v3.0.0 — Character Device Layer
 * ─────────────────────────────────
 * Adds /dev/sensorhub so user-space applications can read the latest
 * decoded sensor values with a plain open()+read() without any
 * additional daemons or middleware.
 *
 ...
 *  │   read(fd, &data, sizeof(data));   ← returns struct sensor_data     │
 *  │   /* data.temperature, .humidity, .pressure, .lux are ready */      │
 *  └─────────────────────────┬────────────────────────────────────────────┘
 *                            │ copy_to_user()  [Section 5 — cdev .read]
 *  ┌─────────────────────────▼────────────────────────────────────────────┐
 *  │  struct sensor_data  priv->cache                                     │
 *  │  Protected by priv->cache_lock  (mutex)                              │
 *  │                                                                      │
 *  │  Writers : sensorhub_do_poll()  (kthread, every 1 s)                │
 *  │  Readers : sensorhub_cdev_read() (any user process calling read())  │
 *  └─────────────────────────▲────────────────────────────────────────────┘
 *                            │ mutex_lock → update → mutex_unlock
 *  ┌─────────────────────────┴────────────────────────────────────────────┐
 *  │  kthread  sensorhub_poll_fn  [Section 4]                             │
 *  │    1. Build FC03 request frame  (rtu_build_read_regs_request)        │
 *  │    2. sensorhub_send_request()                                       │
 *  │       a. gpio HIGH  →  usleep(TX_ENABLE_DELAY)                      │
 *  │       b. serdev_device_write()                                       │
 *  │       c. serdev_device_wait_until_sent()  ← kernel tcdrain()        │
 *  │       d. usleep(TX_DONE_GUARD)  →  gpio LOW                         │
 *  │    3. wait_for_completion_timeout()                                  │
 *  │    4. rtu_parse_read_regs_response()  +  CRC16 verify               │
 *  │    5. mutex_lock → write cache → mutex_unlock                        │
 *  └─────────────────────────▲────────────────────────────────────────────┘
 *                            │ complete(&priv->rx_done)
 *  ┌─────────────────────────┴────────────────────────────────────────────┐
 *  │  receive_buf()  [Section 2 — Serdev RX callback, workqueue ctx]     │
 *  │    spin_lock → accumulate bytes → detect frame complete → spin_unlock│
 *  └──────────────────────────────────────────────────────────────────────┘
 *
 * Device Tree binding (add to your Yocto .dts / .dtsi)
 * ─────────────────────────────────────────────────────
 *  &uart0 {
 *      status = "okay";
 *      rs485_sensor: rs485@0 {
 *          compatible    = "mycompany,rs485-sensorhub";
 *          rts-gpios     = <&gpio 18 GPIO_ACTIVE_HIGH>;
 *          current-speed = <9600>;
 *      };
 *  };
 *
 * Kernel   : Linux 5.15 (Yocto Kirkstone)
 * Hardware : Raspberry Pi 4 + MAX485 transceiver on GPIO 18
 ...
 */
static size_t sensorhub_receive_buf(struct serdev_device *serdev,
                                    const u8             *data,
                                    size_t                count)
{
